...
public record PaginateBrandsQuery(int pageNumber, int pageSize, string? nameField) : IQuery<PaginationResponse<BrandResponse>>;
...
public sealed class PaginateBrandsQueryHandler(IBrandRepository brandRepository) : IQueryHandler<PaginateBrandsQuery, PaginationResponse<BrandResponse>>
{
    ...
    {
        ICollection<BrandResponse> brandResponses = await brandRepository.Paginate(request.pageNumber, request.pageSize, request.nameField);
